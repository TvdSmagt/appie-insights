...
`run.sh` auto-detects how to run: native processes when a compatible toolchain is present (Go 1.23+ and Python 3.12+), otherwise Docker Compose. Force a mode with `--local` or `--docker`.
